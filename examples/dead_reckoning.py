...
class DeadReckoningResult(TypedDict):
    model_name: str
    family: str
    vendor: str
    grade: str
...
def run_case(profile_path: Path, duration: float = 10.0, seed: int = 0) -> DeadReckoningResult:
    ...
    return {
        ...
        "sample_period_s": dt,
        "duration_s": duration,
        "final_position_error_m": float(norm(estimated_position - truth_position[-1])),
        "final_velocity_error_mps": float(norm(estimated_velocity - truth_velocity[-1])),
        "final_attitude_error_deg": float(rad2deg(norm(attitude_error))),
    }
####
...
def main() -> int:
    ...
    for row in rows:
        print(
            f"{row['model_name']:14s} position={row['final_position_error_m']:.6g} m  "
