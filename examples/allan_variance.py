...
def allan_deviation(
    ...
    if cluster_sizes is None:
        ...
        cluster_sizes = unique(geomspace(1, maximum, num=24).astype(int))
    ####
    taus: list[float] = []
...
def cluster_sizes_for_scale(
    ...
    ####
    return unique(geomspace(minimum / sample_period, maximum / sample_period, num=points).astype(int).clip(1))
####
...
def collect_profile_rates(
    ...
    model = ImuModel(config, rng=random.default_rng(seed))
    ...
    return identity, asarray(accelerometer), asarray(gyroscope), dt, parameters
####
...
def main() -> int:
    ...
    parser.add_argument(
        "--tau-max-s", type=float, default=None, help="maximum Allan cluster period; record length limits this"
    )
    parser.add_argument("--points", type=int, default=24, help="number of logarithmic cluster periods")
    parser.add_argument(
        "--profile", type=Path, action="append", default=[], help="additional JSON/YAML profile to include"
    )
    parser.add_argument("--no-plot", action="store_true", help="write CSV data without generating charts")
