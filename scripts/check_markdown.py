...
FORBIDDEN = re.compile(r"INBOX/|research/sources/|manual|source document", re.IGNORECASE)
LINK = re.compile(r"(?<!!)\[[^]]*]\(([^)]+)\)")
...
def markdown_files() -> list[Path]:
    return sorted(
        ...
        if ".git" not in path.parts and ".pytest_cache" not in path.parts and ".venv" not in path.parts
        and not is_gitignored(path.relative_to(ROOT).as_posix())
    )
####
...
def link_target_error(target: str) -> str | None:
    """Return a portability error for a non-relative Markdown link target."""
    if "\\" in target:
        return "link uses platform-specific backslash separators"
    ####
    parsed = urlsplit(target)
    if parsed.scheme or target.startswith(("/", "//")):
        return "link is not relative"
    ####
    return None
####
...
def check() -> list[str]:
    ...
    for relative in files:
        ...
        if format_markdown_text(text) != text:
            errors.append(f"{relative}: Markdown whitespace or block spacing is not normalized")
        ####
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            for match in LINK.finditer(line):
                ...
                target_error = link_target_error(target)
                if target_error:
                    errors.append(f"{relative}:{line_number}: {target_error}: {target}")
                    continue
                ####
